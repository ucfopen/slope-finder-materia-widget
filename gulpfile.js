...
var widget = sanitize("slope-finder");
...
// a combined script tag referencing a single player.js/creator.js source
...
function minifyJs(htmlName)
{
    ...
    data.toString().replace(/<script.*src=\"(?!assets\/vendor\/jsxgraphcore|assets\/vendor\/jquery-1.12.3.min.js|assets\/vendor\/mathquill-0.9.4\/mathquill.min.js|assets\/vendor\/angular.min.js)(.*)"(.*)>/g, function(toreplace)
	{
		gutil.log('toreplace: ', toreplace);
		...
		if(toreplace.indexOf("data-embed=\"false\"") != -1) return toreplace;
        if(toreplace.indexOf("jsxgraphcore") != -1) return toreplace;
...
// a combined link tag referencing a single player.css/creator.css source
...
function replaceScriptAssets(htmlName)
{
	return gulp.src(sourceString + '.build/' + htmlName + '.html')
		.pipe( print() )
        .pipe(replace(/<script.*src=\"(?!assets\/vendor\/jsxgraphcore|assets\/vendor\/jquery-1.12.3.min.js|assets\/vendor\/mathquill-0.9.4\/mathquill.min.js)(.*)"(.*)>/g, function(toreplace)
		{
			if(toreplace.indexOf("//") != -1) return toreplace;
			if(toreplace.indexOf("materia.") != -1) return toreplace;
			if(toreplace.indexOf("data-embed='false'") != -1) return toreplace;
